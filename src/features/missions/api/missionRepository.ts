...
export async function getDailyMission(dateKey: string): Promise<DailyMission> {
  const { data, error } = await getSupabaseClient().rpc('get_daily_mission', { p_challenge_date: dateKey });
...
export async function getMissionRevealPalette(dateKey: string): Promise<MissionRevealColor[]> {
  const { data, error } = await getSupabaseClient().rpc('get_mission_reveal_palette', { p_challenge_date: dateKey });
