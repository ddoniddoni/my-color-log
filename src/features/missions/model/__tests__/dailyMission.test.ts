...
import { getRevealTargetRotation } from '@/src/features/missions/model/missionRevealWheel';
...
describe('mission reveal state', () => {
  ...
  it('accepts twelve distinct server-provided wheel colors', () => {
    const palette = Array.from({ length: 12 }, (_, position) => ({
    ...
    expect(parseMissionRevealPalette(palette)).toHaveLength(12);
    expect(() => parseMissionRevealPalette([...palette.slice(0, 11), palette[0]])).toThrow('Invalid mission reveal palette response');
  });

  it('lands a selected wheel segment under the pointer after full turns', () => {
    expect(getRevealTargetRotation(0)).toBe(1_785);
    expect(getRevealTargetRotation(11)).toBe(1_455);
    expect(() => getRevealTargetRotation(12)).toThrow('invalid_reveal_wheel_slot');
  });
});
