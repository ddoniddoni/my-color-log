  ...
  return (
    <Modal animationType="fade" onRequestClose={() => !isSaving && onClose()} statusBarTranslucent transparent visible={target !== null}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.overlay}>
        <Pressable accessibilityLabel={t('메모 편집 닫기')} accessibilityRole="button" accessibilityState={{ disabled: isSaving }} disabled={isSaving} onPress={onClose} style={StyleSheet.absoluteFill} />
...
function DiaryEditForm({ isSaving, onClose, onSave, target }: Omit<DiaryEditModalProps, 'target'> & { target: DiaryEditTarget }) {
  ...
  const title = t(isPhotoCaption ? '사진 메모 수정' : '오늘의 메모');
  const description = t(isPhotoCaption ? '사진에 남길 짧은 문장을 적어 보세요.' : '그날의 장면이나 기분을 한 줄로 남겨 보세요.');
  ...
  const save = async (): Promise<void> => {
    ...
    try {
      ...
      setErrorMessage(t('메모를 저장하지 못했어요. 연결을 확인한 뒤 다시 시도해 주세요.'));
    }
  };
  ...
  return (
    <View accessibilityViewIsModal style={styles.card}>
          <View style={styles.header}>
            ...
            <Pressable accessibilityLabel={t('메모 편집 닫기')} accessibilityRole="button" accessibilityState={{ disabled: isSaving }} disabled={isSaving} hitSlop={4} onPress={onClose} style={styles.closeButton}><CloseIcon /></Pressable>
          </View>
          ...
          <TextInput
            ...
            placeholder={t(isPhotoCaption ? '예: 비 온 뒤 창가에 남은 색' : '예: 퇴근길에 만난 따뜻한 주황')}
          ...
          <View style={styles.actions}>
            <Pressable accessibilityLabel={t('메모 편집 취소')} accessibilityRole="button" accessibilityState={{ disabled: isSaving }} disabled={isSaving} onPress={onClose} style={({ pressed }) => [styles.cancelButton, pressed && styles.pressed, isSaving && styles.disabled]}><AppText style={styles.cancelText}>취소</AppText></Pressable>
            <Pressable accessibilityLabel={t('메모 저장')} accessibilityRole="button" accessibilityState={{ busy: isSaving, disabled: isSaving }} disabled={isSaving} onPress={() => void save()} style={({ pressed }) => [styles.saveButton, pressed && styles.pressed, isSaving && styles.disabled]}><AppText localize={false} style={styles.saveText}>{isSaving ? t('저장 중…') : t('저장')}</AppText></Pressable>
          </View>
    </View>
  );
}
