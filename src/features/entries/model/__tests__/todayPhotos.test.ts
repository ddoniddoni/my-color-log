...
describe('today photos', () => {
  it('prefers the preserved local image while the same remote photo exists', () => {
    ...
    expect(merged).toEqual([{ id: 'photo-1', uri: queuedPhoto.localUri, position: 1, capturedAt: queuedPhoto.capturedAt, status: 'syncing' }]);
  });

  it('returns the first open position and stops at nine', () => {
    const photos = Array.from({ length: 8 }, (_, index) => ({ id: String(index), uri: 'file:///photo.jpg', position: index + 1, capturedAt: queuedPhoto.capturedAt, status: 'synced' as const }));
  ...
  it('places a first photo in the top-left cell of the fixed nine-cell board', () => {
    const firstPhoto = { id: 'photo-1', uri: 'file:///photo-1.jpg', position: 1, capturedAt: queuedPhoto.capturedAt, status: 'synced' as const };
