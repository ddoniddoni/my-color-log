...
import { colors, spacing, type ThemeColors } from '@/src/design/tokens';
...
export function CameraCaptureScreen({ captureContext }: { captureContext: CaptureContext }) {
  ...
  if (!permission) return <View style={[styles.cameraPage, { backgroundColor: theme.colors.surfaceMuted }]} />;
  ...
  const takePhoto = async (): Promise<void> => {
    ...
    if (sessionState.status !== 'ready' || !sessionState.session) {
      setErrorMessage(t('로그인 정보를 확인하지 못했어요. 오늘 화면에서 다시 시도해 주세요.'));
      return;
    }
    setIsCapturing(true);
    ...
    try {
      ...
      setErrorMessage(t('사진을 찍지 못했어요. 잠시 후 다시 시도해 주세요.'));
  ...
  return (
    <View style={styles.cameraPage}>
      <CameraView
        ...
        onMountError={() => setErrorMessage(t('카메라를 시작하지 못했어요.'))}
        ref={cameraRef}
      ...
      <View style={[styles.cameraTopBar, { paddingTop: Math.max(insets.top, spacing[3]) }]}>
        ...
        <CameraControl accessibilityLabel={language === 'ko' ? `플래시 ${flash}` : `Flash ${flash}`} onPress={() => setFlash(getNextFlashMode(flash))}><FlashIcon mode={flash} /></CameraControl>
      </View>
      <View pointerEvents="none" style={styles.squareGuideArea}>
        <View accessible accessibilityLabel={t('정사각형 사진 저장 가이드')} accessibilityRole="image" style={styles.squareGuide}>
...
function CameraPermissionPrompt({ canAskAgain, onCancel, onOpen }: { canAskAgain: boolean; onCancel: () => void; onOpen: () => void }) {
  ...
  const styles = useCameraPermissionStyles();
  return (
    <View style={[styles.permissionBackdrop, { paddingTop: insets.top, paddingBottom: insets.bottom }]}>
      ...
      <View accessibilityViewIsModal style={styles.permissionCard}>
        <View style={styles.permissionSketch}>
          <Rect fill={colors.surfaceMuted} height={82} rotation={3} stroke={colors.ink} strokeWidth={1.5} width={72} x={20} y={20} />
          <Path d="M39 67 58 48m-13 22 20-20M39 51l5-5m15 17 5-5" fill="none" stroke={colors.ink} strokeLinecap="round" strokeWidth={4} />
          <Rect fill={colors.surface} height={28} rx={4} rotation={12} stroke={colors.ink} strokeWidth={1.5} width={28} x={76} y={8} />
        ...
        <Pressable accessibilityLabel={t(canAskAgain ? '카메라 권한 허용' : '기기 설정 열기')} accessibilityRole="button" onPress={onOpen} style={({ pressed }) => [styles.permissionButton, pressed && styles.permissionButtonPressed]}>
          <AppText style={styles.permissionButtonText}>{canAskAgain ? '카메라 허용' : '설정 열기'}</AppText>
        </Pressable>
        <Pressable accessibilityLabel={t('카메라 화면 닫기')} accessibilityRole="button" hitSlop={12} onPress={onCancel} style={styles.cancelButton}><AppText style={styles.cancelText}>취소</AppText></Pressable>
      </View>
    </View>
  );
}
...
function CameraControl({ accessibilityLabel, children, onPress }: { accessibilityLabel: string; children: React.ReactNode; onPress: () => void }) {
  const { t } = useAppLanguage();
  return <Pressable accessibilityLabel={t(accessibilityLabel)} accessibilityRole="button" onPress={onPress} style={({ pressed }) => [styles.cameraControl, pressed && styles.shutterPressed]}>{children}</Pressable>;
}
...
function useCameraPermissionStyles() {
  const { colors } = useAppTheme();
  return useMemo(() => createCameraPermissionStyles(colors), [colors]);
}

function createCameraPermissionStyles(colors: ThemeColors) {
  return StyleSheet.create({
    permissionBackdrop: { alignItems: 'center', backgroundColor: colors.surfaceMuted, flex: 1, justifyContent: 'center', overflow: 'hidden', paddingHorizontal: 20 },
    ...
    permissionCardShadow: { backgroundColor: colors.black, height: 560, marginLeft: 4, marginTop: 5, maxWidth: 352, position: 'absolute', transform: [{ rotate: '-1deg' }], width: '100%' },
    permissionCard: { alignItems: 'center', backgroundColor: colors.surface, borderColor: colors.ink, borderWidth: 2, minHeight: 560, maxWidth: 352, paddingBottom: spacing[5], paddingHorizontal: 38, paddingTop: spacing[5], transform: [{ rotate: '-1deg' }], width: '100%' },
    ...
    permissionCopy: { color: colors.textSecondary, fontSize: 11, lineHeight: 16, marginTop: spacing[3], textAlign: 'center' },
    permissionDivider: { backgroundColor: colors.borderStrong, height: 1, marginTop: spacing[6], marginVertical: spacing[5], width: '100%' },
    permissionButton: { alignItems: 'center', backgroundColor: colors.ink, minHeight: 64, justifyContent: 'center', width: '100%' },
    permissionButtonPressed: { opacity: 0.78, transform: [{ scale: 0.97 }] },
    permissionButtonText: { color: colors.surface, fontSize: 14, fontWeight: '700' },
