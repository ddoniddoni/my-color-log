-- Daily mission rows are only created through the authenticated RPC below.
-- The underlying tables deliberately have no client-facing grants or RLS policies.

create index daily_missions_color_id_idx on public.daily_missions (color_id);
