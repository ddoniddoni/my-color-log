create function public.get_active_room_today_board()
returns table (
  room_id uuid,
  room_name text,
  room_emoji text,
...
begin
  ...
  if not found
    or (current_member_joined_at at time zone 'Asia/Seoul')::date > target_date then
    return;
  end if;

  target_mission_id := public.ensure_room_daily_mission(target_room_id, target_date);
  ...
  join public.room_daily_missions as room_mission
    ...
    and room_mission.challenge_date = target_date
  ...
  left join public.daily_entries as entry
    ...
    and entry.date_key = target_date
...
revoke all on function public.get_active_room_today_board() from public;
revoke all on function public.get_active_room_today_board() from anon;
grant execute on function public.get_active_room_today_board() to authenticated;
